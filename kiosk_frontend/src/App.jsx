...
function App() {
            ...
            if (payload.audio_b64) {
                try {
                    ...
                    if (playPromise !== undefined) {
                        playPromise.catch(e => {
                            console.error("DOMException (HTML5 Autoplay blocked or decoding error):", e);
                        });
                    }
                } catch (error) {
                ...
                // Fallback to native Web Speech API TTS
                try {
                    ...
                    if (voices.length === 0) {
                        console.warn("speechSynthesis.getVoices() returned an empty array. Browser voices might still be loading.");
                    }
                    ...
                    if (hindiVoice) {
                        ...
                        console.log("Using Hindi TTS voice:", hindiVoice.name);
                    } else if (voices.length > 0) {
                        console.warn("No Hindi ('hi-IN') TTS voice found on OS. Falling back to default system voice:", voices[0].name);
                        utterance.voice = voices[0];
                    }

                    window.speechSynthesis.speak(utterance);
                } catch (e) {
                    console.error("DOMException (TTS Autoplay blocked or error):", e);
                }
            }
        };
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center p-8">
            <h1 className="text-4xl font-bold mb-2">MediKiosk Triage Assistant</h1>
            <p className="text-gray-400 mb-2">
                Status: <span className={status === 'ACTIVE' ? 'text-green-500' : 'text-yellow-500'}>{status}</span> | Face Detected: <span className={faceDetected ? 'text-green-500' : 'text-red-500'}>{faceDetected ? 'Yes' : 'No'}</span> | Triage: <span className={triageStatus === 'EMERGENCY' ? 'text-red-500 font-bold' : 'text-gray-300'}>{triageStatus}</span>
                {roomId && (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', width: '100%', maxWidth: '1200px', gap: '2rem', alignItems: 'start' }}>
                ...
                {/* LEFT COLUMN: Webcam & QR Code */}
                <div className="grid grid-cols-1 gap-12 h-full w-full min-w-0">
                    <div style={{ backgroundColor: '#000', borderRadius: '0.75rem', overflow: 'hidden', border: '1px solid #374151', position: 'relative', minHeight: '480px', width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    {roomId ? (
                        ...
                        <div className="flex flex-col items-center justify-start">
                            <p className="text-gray-500 text-sm text-center">Awaiting Session...</p>
                        </div>
                    )}
                </div>
